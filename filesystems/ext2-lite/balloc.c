                               ...
                               struct buffer_head *bitmap_bh, unsigned long *count)
{
	...
	/* ? */
	struct ext2_sb_info *sbi = EXT2_SB(sb);
	first_free_bit = num = find_next_zero_bit_le(bitmap_bh->b_data, nblocks, 0);
	if (num >= nblocks)
		return -1;
	int end = find_next_bit_le(bitmap_bh->b_data, nblocks, num);
	if (*count > end - num)
		*count = end - num;
	for (; num < first_free_bit + *count; num++){
		ext2_set_bit_atomic(sb_bgl_lock(sbi, group), num, bitmap_bh->b_data);
	}
	ext2_debug("allocated %lu blocks in bg %d\n", *count, group);
