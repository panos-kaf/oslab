                             ...
                             struct folio **foliop)
{
	...
	for (i=0; i < npages; i++) {
		...
		if (IS_ERR(kaddr)){
			ext2_debug("ext2_get_folio failed\n");
			return ERR_CAST(kaddr);
		}
		de = (ext2_dirent *)kaddr;
		char* last = kaddr + ext2_last_byte(dir, i) - reclen;

		while ((char *)de <= last){
			if (ext2_match(namelen, name, de)){
				ext2_debug("found entry in page %lu of inode %lu\n", i,dir->i_ino);
				return de;
			}
	...
	ext2_debug("entry not found in dir %lu\n",dir->i_ino);
	return ERR_PTR(-ENOENT);
}
